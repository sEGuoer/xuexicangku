...
insert into tb_user(tb_user.name)
values ('x'),
       ('y'),
       ('z');
...
insert into tb_role(tb_role.name)
values ('管理员'),
       ('学生'),
       ('游客');
...
insert into tb_permissions(tb_permissions.name)
values ('教学'),
       ('上课'),
       ('吃饭'),
       ('旁听');
...
(
    ...
    constraint fk_role1 foreign key (role_id) references tb_role (id),
    constraint fk_permission foreign key (permission_id) references tb_permissions (id)
) comment '角色权限中间表';
